...
const AllOrders = () => {
  ...
  const handleStatusChange = (status) => {
    ...
    fetch('http://localhost:3001/updateOrder', {
      method: 'POST',
      headers: {
      ...
      .then((res) => res.json())
      .then((data) => console.log('postorder',data))
  ...
  const onSubmit = (data) => {
    ...
    fetch('http://localhost:3001/updateOrderDetails', {
      method: 'POST',
      headers: {
        'Content-type': 'application/json',
      },
      ...
      .then((res) => res.json())
      .then((data) => console.log('updetil',data))
      .catch((err) => console.log(err));
          ...
          <tbody>
            {ContextData.order.map((item) => (
              <tr key={item._id}>
                ...
                <td className="text-center">{item.products.length} buah</td>
                <td>
                  <Button
                ...
                <td className="pt-2" >
                  <select
                    ...
                    onChange={(e) => {setSelectOrder(item);handleStatusChange(e.target.value)}}
                    className={
            ...
            <div className="px-5 my-3 single-order">
              <form className="" onSubmit={handleSubmit(onSubmit)}>
                <div className="">
                  ...
                  <h4 className="modal-title text-primary ml-4">
